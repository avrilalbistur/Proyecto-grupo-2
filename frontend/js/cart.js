...
function calcularTotalConEnvio(tipoEnvio) {
  ...
  switch (tipoEnvio) {
    case "premium":
      ...
      precioEnvio.textContent = `Costos de envío ${moneda} ${costoEnvio.toFixed(
        2
      )}`;
      break;
    case "express":
    ...
    productoDiv.innerHTML = `
      ...
      producto.nombre
        ...
        <div class="detalles-producto">
          ...
          <input type="number" id="cantidad-${index}" min="1" value="${
      producto.cantidad
    }" data-index="${index}" class="cantidad-input">
          <p class="dark-mode-title">SUBTOTAL ${(
            producto.costo * producto.cantidad
          ).toFixed(2)}</p>
          <button onclick="eliminarProducto(${producto.id})" id="${
      producto.id
    }">Eliminar</button>
        </div>
      `;
...
// Función que agrupa a las diversas actualizaciones del carrito
...
function funcionamientoFormulario(event) {
    event.preventDefault(); // Evitar que el formulario se envíe de forma tradicional
      ...
      Swal.fire({
        ...
        if (result.isConfirmed) {
          ...
          actualizarCarritoCompleto();
          console.log(
            "Compra realizada con éxito. El carrito ha sido vaciado y el formulario limpiado."
          );
...
document.addEventListener("DOMContentLoaded", () => {
    ...
    // En caso de que el usuario decida pagar con tarjeta
    document.getElementById("debito").addEventListener("click", () => mostrarOpcion("tarjeta"));

    document.getElementById("credito").addEventListener("click", () => mostrarOpcion("tarjeta"));
});